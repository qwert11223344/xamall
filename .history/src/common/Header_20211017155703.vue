<template>
	<header>
		...
		>
			<router-link class="link" to="/shopcart" ref="cart">
				<i class="iconfont">&#xe602;</i>
			</router-link>
			<transition name="fade">
					...
					<ul>
						...
						>
							<div class="product_item">
								...
								<div class="delete">
									<span @click="_delProd({ prodId: item.prod_id })">
...
export default {
	...
	mounted() {
		...
		let left = this.$refs.cart.$el.getBoundingClientRect().left;
		let top = this.$refs.cart.$el.getBoundingClientRect().top;
		this.$store.commit(RECORD_CARTPOSTION, { left, top });
	},
	created() {
