...
export default {
  ...
  // 加入购物车
  [ADD_CART](state, { productId, price, productTitle, productImg, productNum }) {
    let goods = {
      productId,
      price,
      productTitle,
    ...
    let oldProduct = state.cartList.find((item) => item.productId === goods.productId)
    if (oldProduct) {
      oldProduct.productNum += productNum
    }
