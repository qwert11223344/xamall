	...
	<div class="checkout">
		...
		<div class="w" style="padding-top: 40px">
				...
				<template #content>
					<ul class="address-item-list clearfix">
						<li
							...
							:key="i"
							class="address pr"
							:class="{ checked: currentIndex === i }"
							@click="chooseAddress(item, i)"
						>
							<span v-if="currentIndex === i" class="pa">
			...
			<y-popup :open="popupOpen" @close="popupOpen = false" :title="popupTitle">
				<template #content>
					<el-form :model="msg" status-icon :rules="rules" ref="elForm">
						<div class="md">
							<div>
								<el-form-item prop="nick_name">
									<el-input
										v-model.lazy="msg.nick_name"
										clearable
										maxlength="10"
										show-word-limit
										placeholder="收货人姓名"
...
export default {
	...
	methods: {
		...
		update(item) {
			...
			if (item) {
				...
				Object.keys(this.msg).forEach((v) => (this.msg[v] = ""));
				this._getRegion();
			}
		},
		// 重置
		...
		// 提交订单后跳转付款页面
		_submitOrder() {
			this.submitText = "提交订单中...";
			this.submit = true;
			if (this.addList.length === 0) {
				this.message("当前还没有收获地址，请添加收获地址哦！", "error");
				this.reset();
				return false;
			}
			let params = {
				...
				address: this.addList[this.currentIndex],
	...
	created() {
		...
		this.cartList = this.$route.params.checkList;
		this._getAddressList();
	},
	watch: {
		cartList() {
			if (!this.cartList) {
				this.message("请重新选择商品", "error");
				this.$router.back();
				if ("/login".indexOf(this.$route.path)) {
					this.$router.back();
				}
			}
		},
