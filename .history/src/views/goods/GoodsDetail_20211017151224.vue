...
import { ADD_CART, ADD_ANIMATION } from "store/mutation-types";
export default {
	name: "GoodsDetail",
	data() {
		return {
	...
	computed: mapState({
		login: "login",
		userInfo: "userInfo",
		cartPositionT: "cartPositionT",
		cartPositionL: "cartPositionL",
		showMoveImg: "showMoveImg",
	...
	methods: {
		...
		addCart(event) {
			if (!this.login) {
				...
				let params = {
					...
					prodTitle:
						this.selectProd.product_title || this.product.product_title,
				...
				addToCart(params).then((res) => {
					if (res.success === true) {
						...
						if (!this.showMoveImg) {
							...
							// 获取点击的坐标
							let elLeft =
								dom.getBoundingClientRect().left + dom.offsetWidth / 2;
							let elTop =
								dom.getBoundingClientRect().top + dom.offsetHeight / 2;
							let img = this.product.product_img;
							// 需要触发
		...
		beforeEnter(el) {
			...
			elStyle.transform = `translate3d(0,${
				this.elTop - this.cartPositionT
			}px,0)`;
