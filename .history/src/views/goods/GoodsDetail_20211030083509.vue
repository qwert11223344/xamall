...
<template>
	...
	>
		...
		<el-backtop target=".goods-detail" visibility-height="20px"></el-backtop>
	</div>
</template>
<script>
...
export default {
	...
	computed: mapState({
		...
		userInfo: "userInfo",
		prod_title() {
			return "product_title" in this.selectProd
	...
	methods: {
		...
		checkout() {
			let checkList = {
				prod_id: this.selectProd.product_id,
				prod_img: this.selectProd.product_img,
				prod_title: this.prod_title,
				prod_price: this.selectProd.price,
				prod_count: this.productNum,
				group_id: this.groupId,
			};
			this.$router.push({
				name: "Checkout",
				params: {
					checkList: Array.of(checkList),
				},
			});
		},
		editNum(arg) {
			this.productNum = arg.num;
		},
		//添加购物车
		addCart() {
			if (!this.login) {
