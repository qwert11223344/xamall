...
export default {
	...
	methods: {
		...
		sortPrice(val) {
			this.sortType = val;
			this.showGoods = this.allGoods.sort((a, b) => {
				return val === 1 ? a.price - b.price : b.price - a.price;
			});
		},
		// 价格筛选
		rangePrice() {
			this.showGoods = this.allGoods.filter((item) => {
		...
		_getAllGoods() {
			...
			getAllGoods().then((res) => {
				if (res.success === true) {
					...
					this.showGoods = this.allGoods.slice(0, this.pageSize);
					this.noResult = false;
					if (this.goodsNum === 0) {
						this.noResult = true;
					}
	...
	computed: {
		goodsNum() {
			return this.showGoods.length < this.pageSize
				? this.showGoods.length
				: this.showGoods.length + (this.currentPage - 1) * this.pageSize;
		},
		showGoods() {
			return this.showGoods * this.currentPage > this.goodsNum
				? this.allGoods.slice((this.currentPage - 1) * this.pageSize)
				: this.allGoods.slice(
