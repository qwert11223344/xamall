		...
		<popup :open="popupOpen" @close="popupOpen = false" :title="popupTitle">
			<template #content>
				<div class="md">
					...
					<div>
						<el-select v-model="msg.downtown_area" placeholder="请选择城市">
							<el-option
								v-for="item in downtownArea"
								:label="item.name"
								:value="item.name"
							/>
						</el-select>
...
export default {
	...
	methods: {
		...
		currentProvince(item) {
			getRegion(item, 2).then((res) => {
				if (res.status === 1) {
					this.downtownArea = res.districts[0].districts;
				}
			});
		},
		_addressList() {
