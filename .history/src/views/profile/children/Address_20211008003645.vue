...
export default {
	data() {
		...
		let validateDowntown = (rule, value, callback) => {
			if (value === "") {
		...
		return {
			...
			rules: {
				...
				downtown_area: [{ validator: validateDowntown, trigger: "blur" }],
				county: [{ validator: validateCountry, trigger: "blur" }],
			},
		};
	},
	components: {
	...
	methods: {
		...
		check(e) {
			...
			// 	e.target.placeholder = "请输入收货人姓名";
			// 	return;
			// } else if (this.msg.tel === "") {
			// 	e.target.placeholder = "请输入联系电话";
			// 	return;
			// } else if (this.msg.streetName === "") {
			// 	e.target.placeholder = "请输入详细地址";
			// 	return;
			// }
		},
		// 获取地址列表
